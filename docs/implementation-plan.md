...
**Export.** `evals/export.py` pulls a slice into `datasets/exports/<timestamp>.jsonl` of `NormalizedItem` records plus a `manifest.json` (server machine id, section ids and agents, plexapi version, exporter git sha, reload-includes used). Determinism comes from: `autoreload=false`, an explicit include set, canonical JSON with sorted keys, and records sorted by `item_id`. Re-running the export against an unchanged library must produce a byte-identical file — that is the export's own test.
...
| 0.4 | Export | `evals/export.py`, `cli.py:export` | `shelfwarden export --count 200` writes a deterministic JSONL + manifest; re-run is byte-identical |
...
- **The real library's problem distribution is unknown.** The 200-item slice may contain very few shows with absolute numbering, or no remake pairs. Step 0.4 should therefore emit a **census** of the export (counts by section, agent type, media kind, container) *before* the corruption step, so the slice composition is chosen from evidence rather than assumed. Expect to revise target counts after seeing the census.
...
- **`locked=True` is plexapi's default on every edit helper.** It pins fields against future agent refreshes. Phase 3's revert must restore lock state, not just values. Recording current lock state in the export now (Step 0.4) avoids an unpleasant discovery later.
